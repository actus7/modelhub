/**
 * ClinePass OAuth provider configuration.
 * Flow: Authorization Code (base64-encoded token in callback)
 * Shares Cline's OAuth endpoints.
 */

import type { BaseProviderConfig, OAuthProviderHandler } from '../types';

const CLINEPASS_CONFIG: BaseProviderConfig = {
  authorizeUrl: 'https://app.cline.bot/auth',
  tokenUrl: 'https://app.cline.bot/api/v1/auth/token',
};

const clinepass: OAuthProviderHandler = {
  ...
  async exchangeToken(config, code, redirectUri) {
    try {
      ...
      if (!response.ok) throw new Error(`ClinePass token exchange failed: ${await response.text()}`);
